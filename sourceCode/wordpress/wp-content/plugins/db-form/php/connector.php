...
//WP LADEN
$folder = getcwd();
$cutted = substr($folder, strpos($folder, 'wp-content')-1);
$path = 'wp-load.php';
for($i=1;$i<=count_character('/',$cutted);$i++){ $path = '../'.$path; }
	...
	if(get_option('dbf-0-main-function') == 'mail'){
		...
		if(mail(get_option('dbf-0-admin-mail'), get_option('dbf-0-mail-subject'), $dbf_mail_content, $dbf_mail_header)){
			...
			if(get_option('dbf-0-confirmation') == 'true'){
				...
				if(mail(get_option('dbf-0-admin-mail'), get_option('dbf-0-confirmation-subject'), $dbf_confirmation_content, $dbf_confirmation_header)){
					...
					if(get_option('dbf-0-redirect') != 'custom' ){
						...
						header("Location: ".$dbf_prev_url.'?'.http_build_query($returns));
						exit;
					}else{
...
function count_character($character,$string){
	$analysis = count_chars($string, 1);
	foreach ($analysis as $char=>$count){
		if($char == ord($character)){
			$throwback = $count;
		}
	}
	return $throwback;
