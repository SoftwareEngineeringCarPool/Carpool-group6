					...
					if ($row['isAvailable'] == 1)
					{
						...
						if ($row['matched']== 1)
						{
							...
							else
							{
								$find = $row['trip_id'];
								echo "<br>";
								$query = 'Select * from users where users.user_id in (Select provider_id from trip_match, tripInfo where taker_trip = ' . $find . ');';
								$result =mysql_query( $query , $link);
								...
								$q2 = 'Select * from tripInfo where tripInfo.trip_id in (Select provider_trip from trip_match, tripInfo where taker_trip = ' . $find . ');';
								$result2 =mysql_query( $q2 , $link);
